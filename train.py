    ...
    with tf.Session() as sess:
        ...
        for e in range(args.num_epochs):
            ...
            for b in range(data_loader.num_batches):
                ...
                y1 = [20]*args.batch_size #pointer labels for start index. Not one-hot.
                y2 = [35]*args.batch_size #pointer labels for end index. Not one-hot
                #paragraphs = np.zeros((args.batch_size, 500))
                feed = {model.input_data: paragraphs, model.targets_start: y1, model.targets_end: y2} #TODO: integrate questions, targets_start, targets_end
                #for i, (c, h) in enumerate(model.initial_states_fw): #for each LSTMStateTuple
                    #feed[c] = state[i].c
                    #feed[h] = state[i].h
                train_loss, p1, p2, _ = sess.run([model.cost, model.p1, model.p2, model.train_op], feed)
                print(p1[0][20], p2[0][35])
                #print(p2)
