...
class AttentionLayer():
    ...
    def __init__(self, p_inputs, q_inputs, rnn_size, scope, batch_size=10, training=True):
        ...
        p_inputs_aug = tf.tile(tf.expand_dims(p_inputs, 2), [1, 1, q_length, 1]) #batch_size x p_length x q_length x rnn_size
        q_inputs_aug = tf.tile(tf.expand_dims(q_inputs, 1), [1, p_length, 1, 1]) #batch_size x p_length x q_length x rnn_size
        pq_elementwise = tf.multiply(p_inputs_aug, q_inputs_aug) #batch_size x p_length x q_length x rnn_size
        combo_input = tf.concat([p_inputs_aug, q_inputs_aug, pq_elementwise], axis=3) #batch_size x p_length x q_length x 3*rnn_size

        with tf.variable_scope(scope):
            w_sim = tf.get_variable("w_sim", [3*rnn_size, 1])

        #in order to matmul combo_input with w_sim, we need to first tile w_sim batch_size number of times, and flatten combo_input
        combo_input_flat = tf.reshape(combo_input, [batch_size, -1, 3*rnn_size]) #batch_size x p_length*q_length x 3*rnn_size
        w_sim_tiled = tf.tile(tf.expand_dims(w_sim, 0), [batch_size, 1, 1]) #batch_size x 3*rnn_size x 1
        ...
        #a #for each p, find weights to put on q. ##batch_size x p_length x q_length x rnn_size
        att_on_q_aug = tf.tile(tf.nn.softmax(sim_mtx, dim=2), [1, 1, 1, rnn_size])
        #q_inputs_aug = batch_size x p_length x q_length x rnn_size
        weighted_q = tf.multiply(att_on_q_aug, q_inputs_aug)
        linear_combo_q_for_each_p = tf.reduce_sum(weighted_q, axis=2) #batch_size x p_length x rnn_size
        ...
        weights_p = tf.tile(att_on_p, [1, 1, rnn_size]) #batch_size x p_length x rnn_size
        weighted_p = tf.multiply(weights_p, p_inputs) #batch_size x p_length x rnn_size
