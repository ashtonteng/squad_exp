...
class PointerLayer():
    def __init__(self, inputs, scope, num_layers=1, batch_size=10, rnn_size=128, input_keep_prob=1.0, 
        ...
        with tf.variable_scope(scope):
            w_p1 = tf.get_variable("w_p1", [input_shape[1], 854]) # 1000 is random
            w_p2 = tf.get_variable("w_p2", [input_shape[1], 854])

        self.p1 = tf.nn.softmax(tf.matmul(inputs, w_p1))
        self.p2 = tf.nn.softmax(tf.matmul(inputs, w_p2))
        ...
        targets_start_onehot = tf.one_hot(targets_start_reshaped, depth=854, on_value=1.0, off_value=0.0)
        targets_end_onehot = tf.one_hot(targets_end_reshaped, depth=854, on_value=1.0, off_value=0.0)
